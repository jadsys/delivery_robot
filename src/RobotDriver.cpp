...
* @brief    RobotDriverクラスのソースファイル
* @author   S.Kumada
* @note     ロボットの操作（並進・旋回）を行うクラスの実装
*/
...
public:
  ...
  /**
  * @brief   RobotDriverクラスのコンストラクタ
  * @param[in]   ros::NodeHandle &nh ノードハンドル
  * @details 初期化を行う
  */
  ...
  * @brief   RobotDriverクラスのコンストラクタ
  * @param[in]   ros::NodeHandle &nh ノードハンドル
  * @param[in]   ros::NodeHandle &privateNode パラメータ用ノードハンドル
  * @details 初期化を行う
  */
...
//      current_transform.getOrigin().z(), current_transform.getOrigin().w());
  ...
  /**
  * @brief   指定角度旋回処理
  * @param[in]   bool clockwise 旋回方向
  * @param[in]   double radians　旋回角度
  * @param[in]   double angularSpeed = 0.4　旋回速度
  */
  ...
  */
  ...
  /**
  * @brief   指定速度並進処理
  * @param[in]   double linearSpeed　並進速度
  */
  void moveForward(double linearSpeed)
  { 
      geometry_msgs::Twist base_cmd;
      base_cmd.linear.y = base_cmd.angular.z = 0;
  ...
  * @brief   指定速度旋回処理
  * @param[in]   bool clockwise　旋回方向
  * @param[in]   double angularSpeed　旋回速度
  */
  void moveTurn(bool clockwise, double angularSpeed)
  { 
    geometry_msgs::Twist base_cmd;
    base_cmd.linear.x = base_cmd.linear.y = 0.0;
    base_cmd.angular.z = angularSpeed;
