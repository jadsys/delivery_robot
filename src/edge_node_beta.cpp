/**
* @file     edge_node_beta.cpp
* @brief    ダミー上位系のソースファイル
* @author   S.Kumada
* @note     ロボットへのナビゲーション・一時停止・再開・キャンセルなどの指示を行う
*/
...
#include "uoa_poc3_msgs/r_navi_result.h"
#include "uoa_poc3_msgs/r_navi_command.h"
...
ros::Subscriber subs_correct_position; // 補正値情報のメッセージ用サブスクライバ

ros::Publisher para_pub;
ros::Publisher para_emg;
ros::Publisher para_costmap;
...
void recvCorrectPosition(const geometry_msgs::PoseStamped& curr_pose)
{
    isRecvCorrectPosition = true;
    curr_pose_ = curr_pose;
}

void recvRobotState(const uoa_poc3_msgs::r_state& msg)
{
    /*****************************
    * string id
    * string type
...
void recvinfo(const uoa_poc3_msgs::r_info& msg)
{
    /***************************************
    * string id
    * string type
...
void recvNaviCMDResult(const uoa_poc3_msgs::r_navi_result& msg)
{
    /****************************************
    * string id
    * string type
    ...
    *****************************************/
    ...
    ROS_INFO("id(%s) type(%s) time(%s) received_revision (%s) received_time(%s) received_cmd(%s) result(%s) errors.size()(%d)",msg.id.c_str(), msg.type.c_str(), msg.time.c_str(), msg.received_revision.c_str(), msg.received_time.c_str(), msg.received_cmd.c_str(), msg.result.c_str(), (int)msg.errors.size());
...
void recvEmergencyCMDResult(const uoa_poc3_msgs::r_emergency_result& msg)
{
    /****************************************
    * string id
    * string type
...
int main(int argc, char **argv)
{
  ...
  //"1000"はトピックキューの最大値
    ...
    // パブリッシャ
    para_pub = n.advertise<uoa_poc3_msgs::r_navi_command>("/robot_bridge/"+ entity_id +"/navi_cmd", 100, true);
    para_emg = n.advertise<uoa_poc3_msgs::r_emergency_command>("/robot_bridge/"+ entity_id +"/emg", 100, true);
    para_costmap = n.advertise<nav_msgs::OccupancyGrid>("/cost_map_pointed", 100, true);
    ...
    subs_correct_position   = n.subscribe("/correction_info", 100, &recvCorrectPosition);        // stateメッセージのサブスクライバ

    // リビジョン番号
    std::string revision;
    if (paramNode.getParam("map_revision", revision))
    {
        ROS_INFO("revision (%s)", revision.c_str());
    }
    else
    {
        revision = "cfz7-f4z2-siyb-gedc-2";

        ROS_WARN("param not found : revision (%s)", revision.c_str());
    }
...
// tst
    ...
    //uoa_poc3_msgs::r_req型のオブジェクトを定義
    uoa_poc3_msgs::r_navi_command msg;
    uoa_poc3_msgs::r_emergency_command emg_msg; // 緊急停止コマンド
...
// 20221226 test
            ...
            msg.destination.angle_optional.valid = true;
            msg.destination.angle_optional.angle.yaw = 1.57;
            msg.revision            = "cfz7-f4z2-siyb-gedc-1";
            break;
...
//
        ...
        case 152:// navi   B	ロッカー前１
            ...
            msg.destination.point.x = -2.184;
            msg.destination.point.y =  1.002;
            msg.destination.point.z =  0.0;
            msg.destination.angle_optional.valid = true;
            msg.destination.angle_optional.angle.yaw = -1.57;
            break;
        ...
        case 153:// navi   C	ロッカー前２
            ...
            msg.destination.point.x = -3.214;
            msg.destination.point.y =  1.013;
            msg.destination.point.z =  0.0;
            msg.destination.angle_optional.valid = true;
            msg.destination.angle_optional.angle.yaw = -1.57;
            break;
    ...
    // 20231016 ロボット動作確認
        case 167:// navi   	RDR合同
            ...
            msg.destination.point.x =  0.2566;
            msg.destination.point.y =  7.717;
            msg.destination.point.z =  0.0;
            msg.destination.angle_optional.valid = true;
            msg.destination.angle_optional.angle.yaw =  1.57;
        ...
        case 168:// 環境地図 事務室前
            ...
            msg.destination.point.x =  1.9588695764541626;
            msg.destination.point.y =  -9.128177642822266;
            msg.destination.point.z =  0.0;
            msg.destination.angle_optional.valid = true;
            msg.destination.angle_optional.angle.yaw =  1.57;
        ...
        case 169: // 環境地図 RDR合同
            ...
            msg.destination.point.x =  -0.6546769738197327;
            msg.destination.point.y =  -3.2333762645721436;
            msg.destination.point.z =  0.0;
            msg.destination.angle_optional.valid = true;
            msg.destination.angle_optional.angle.yaw =  1.57;
    ...
    //社内テスト用コード開始（2020/09/30実施）
        ...
        if(isRecvCostmap)
        {
            ...
            // 目的地をプロットした配信用マップの作成
            nav_msgs::OccupancyGrid pub_map = plan_costmap;

            // 座標値変換
            unsigned int mx = (int) ((msg.destination.point.x - pub_map.info.origin.position.x) / pub_map.info.resolution);
            unsigned int my = (int) ((msg.destination.point.y - pub_map.info.origin.position.y) / pub_map.info.resolution);
            
            // インデックス計算
            unsigned int idx = my * pub_map.info.width + mx;

            // セット
            pub_map.data[idx] = -111;

            para_costmap.publish(pub_map);
        }

        // 座標位置補正
        if(isRecvCorrectPosition)
        {
            /***************************
             * 回転の行列式
             * (X)     (cosθ  −sinθ) (x)
             ...
             * 
            ****************************/
            double x, X, y, Y, roll, pitch, yaw, theta;
            
            x = msg.destination.point.x; 
            y = msg.destination.point.y;

            // 姿勢情報のQuaternion⇒オイラー角を計算する
            geometry_msgs::Quaternion orientation = curr_pose_.pose.orientation;
            tf::Matrix3x3 mat(tf::Quaternion(orientation.x, orientation.y, orientation.z, orientation.w));
            mat.getEulerYPR(yaw, pitch, roll);
            theta = -yaw;

            ROS_INFO_STREAM("prevX:" << x << " prevY:" << y << " prevYaw:" << msg.destination.angle_optional.angle.yaw);

            // 回転の公式から回転後の座標を求める
            X = (x + curr_pose_.pose.position.x) * cos(theta) - (y + curr_pose_.pose.position.y) * sin(theta);
            Y = (x + curr_pose_.pose.position.x) * sin(theta) + (y + curr_pose_.pose.position.y) * cos(theta);
            
            // 修正した値を入れ直す
            msg.destination.point.x = X;
            msg.destination.point.y = Y;

            // ロボットの回転角度も調整する
            if(msg.destination.angle_optional.valid)
            {
                msg.destination.angle_optional.angle.yaw += theta;
            }

            ROS_INFO_STREAM("correctX:" << curr_pose_.pose.position.x << " correctY:" << curr_pose_.pose.position.y << " correctYaw:" << yaw);
            ROS_INFO_STREAM("newX:" << X << " newY:" << Y << " newYaw:" << msg.destination.angle_optional.angle.yaw);
        }
